...
export default function () {
  const { activePlayer, players, map, activePlayerIndex, isGameStarted, turn, timer } = storeToRefs(useGameStore());
  const { addLog } = useLogger();

  const startPlacement = () => {
    addLog(`Расстановка бойцов`);
  ...
  const checkPlacementStatus = () => {
    ...
    if (currentPlayerDone) {
      addLog(
        `Все бойцы игрока ${activePlayer.value.name} расставлены. Подтвердите готовность:`, 
        'system', 
  ...
  return { placeUnit, availableSpawnPoints, startPlacement };
}
