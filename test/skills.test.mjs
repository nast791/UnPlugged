import { medusaSkill, teslaSkill } from './skillFixtures.mjs';
import { EFFECT_TRIGGERS } from '../shared/constants/triggers.js';
import { ITEM_STATES } from '../shared/constants/items.js';
import { runHeroSkills, turnStart } from '../shared/utils/phases/turnstart.js';
import { startGame } from '../shared/utils/phases/startgame.js';
import { turnEnd } from '../shared/utils/phases/turnend.js';
import { evaluateTrigger, pickPipelineCell, pickPipelineTarget, submitPipelineInput, startPipeline } from '../shared/utils/rules/pipeline.js';
import { runFact } from '../shared/utils/rules/facts.js';
import { game } from '../shared/utils/game.js';
import { assert, makeCtx, makeMap, mockEvents, playCtx, withCurrentHp } from './fixtures.mjs';
...
const normalizePlayers = players =>
  players.map(p => ({ ...p, fighters: p.fighters?.map(withCurrentHp) }));
...
// --- medusa: evaluateTrigger ---
{
  ...
  assert(triggered?.actions?.length === 4, 'evaluateTrigger returns hook with actions');
...
// --- medusa: full apply flow ---
{
  ...
  moves.selectTarget(playCtx, { fighterId: 'enemy1' });
  assert(G.players[1].fighters[0].currentHp === 4, 'medusa skill damage');
...
// --- medusa: skip gaze ---
{
  ...
  assert(G.players[1].fighters[0].currentHp === 5, 'skip gaze: no damage');
  assert(G.pipeline === null, 'medusa skip pipeline done');
  assert(events.endPhaseCalled, 'END_PHASE after skip');
}

// --- medusa: shuffled gaze actions ---
{
  const G = makeMedusaG();
  const events = mockEvents();
  const playCtx = { G, ctx, events };
  const shuffled = [...medusaHook.actions].reverse();

  evaluateTrigger(EFFECT_TRIGGERS.START_TURN, medusaHook, G, ctx);
  startPipeline(G, ctx, events, shuffled, medusaSkill.id);
  submitPipelineInput(playCtx, { vars: [{ var: '$answer', value: 'apply' }] });
  assert(G.targetSelection?.kind === 'target', 'shuffled: target selection pending');

  pickPipelineTarget(playCtx, 'enemy1');
  assert(G.players[1].fighters[0].currentHp === 4, 'shuffled gaze damage');
  assert(G.pipeline === null, 'shuffled medusa pipeline done');
}
...
// --- tesla: START_TURN discharge (damage + optional push) ---
{
  ...
  assert(G.players[1].fighters[0].currentHp === 4, 'tesla discharge damage');
  assert(G.pendingActions.some(a => a.action === 'setVariables'), 'move prompt pending');
  assert(String(G.players[1].fighters[0].position) === '2', 'no move before choice');

  submitPipelineInput(playCtx, { vars: [{ var: '$answer', value: 'skip' }] });
  assert(String(G.players[1].fighters[0].position) === '2', 'skip move keeps position');
...

{
  ...
  assert(pickPipelineTarget(playCtx, 'enemy1'), 'pick discharge target');
  assert(pickPipelineCell(playCtx, '4'), 'move discharge target to cell 4');
  assert(String(G.players[1].fighters[0].position) === '4', 'discharge move updates position');
  assert(G.pipeline === null, 'tesla move pipeline done');
}
...
// --- tesla: discharge with shuffled actions ---
{
  const G = makeTeslaG();
  G.players[0].items = makeCoils([ITEM_STATES.ACTIVE, ITEM_STATES.ACTIVE]);
  const events = mockEvents();
  const playCtx = { G, ctx, events };
  const shuffled = [...teslaDischargeHook.actions].reverse();

  evaluateTrigger(EFFECT_TRIGGERS.START_TURN, teslaDischargeHook, G, ctx);
  startPipeline(G, ctx, events, shuffled, teslaSkill.id);
  assert(G.players[1].fighters[0].currentHp === 4, 'shuffled discharge damage');
  submitPipelineInput(playCtx, { vars: [{ var: '$answer', value: 'skip' }] });
  assert(G.pipeline === null, 'shuffled discharge pipeline done');
}
